...
vols.addEventListener("input", e => {
    ...
    vols.style.background = `linear-gradient(to right, rgb(190, 102, 0) ${volPerc}%, rgb(190 102 0 / 58%) ${volPerc}%)`
...
function tt() {
    vc.classList.toggle("tha");
    if(!localStorage.getItem("tha")) {
        localStorage.setItem("tha", true)
    } else if(localStorage.getItem("tha") === "true") {
        localStorage.setItem("tha", false)
    } else if (localStorage.getItem("tha") === "false") {
        localStorage.setItem("tha", true)
    }
}
